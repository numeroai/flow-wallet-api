...
func runServer(cfg *configs.Config) {
	...
	fc, err := grpc.NewBaseClient(
		cfg.AccessAPIHost,
		grpcOpts.WithTransportCredentials(insecure.NewCredentials()),
		grpcOpts.WithDefaultCallOptions(grpcOpts.MaxCallRecvMsgSize(cfg.GrpcMaxCallRecvMsgSize)),
	)
	...
	rv.Handle("/accounts/{address}/revoke-key/{index}", accountHandler.AddNewKey()).Methods(http.MethodPost) // add new key
