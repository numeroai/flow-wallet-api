...
// this is the ExampleToken from https://github.com/onflow/flow-ft/blob/master/contracts/ExampleToken.cdc
...
access(all) contract FlowToken: FungibleToken {
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        /// elsewhere.
        ///
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @FlowToken.Vault {
    ...
    init() {
        ...
        //
        let exampleTokenCap = self.account.capabilities.storage.issue<&FlowToken.Vault>(self.VaultStoragePath)
        self.account.capabilities.publish(exampleTokenCap, at: self.VaultPublicPath)
        let receiverCap = self.account.capabilities.storage.issue<&FlowToken.Vault>(self.VaultStoragePath)
